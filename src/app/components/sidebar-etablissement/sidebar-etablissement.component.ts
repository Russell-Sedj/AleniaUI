...
@Component({
  selector: 'app-sidebar-etablissement',
  standalone: true,
  imports: [CommonModule, AvatarComponent],
  template: `
    <aside class="flex w-72 flex-col bg-gray-900 border-r border-gray-800 shadow-lg lg:fixed lg:inset-y-0 lg:z-50">
      <!-- Header avec logo -->
      ...
      <!-- Footer avec profil établissement -->
      <div class="border-t border-gray-800 p-4" *ngIf="etablissementProfile">
        <div class="flex items-center gap-3 p-3 bg-gray-800 rounded-lg">
          <app-avatar 
            [initials]="etablissementProfile.initials" 
            size="sm"
            className="bg-gradient-to-r from-green-500 to-emerald-500 text-white">
          </app-avatar>
          <div class="min-w-0 flex-1">
            <p class="text-sm font-medium text-white truncate">{{ etablissementProfile.nom }}</p>
            <p class="text-xs text-gray-400 truncate">{{ etablissementProfile.responsable }}</p>
          </div>
        </div>
      </div>
    </aside>
...
export class SidebarEtablissementComponent {
  @Input() logoSrc = '/assets/images/logo.png';
  @Input() companyName = 'ALENIA';
  @Input() subtitle = 'Espace Établissement';
  @Input() navigationItems: NavigationItemEtablissement[] = [];
  @Input() activeItem: string = '';
  @Input() etablissementProfile: EtablissementProfile | null = null;
  @Input() showLogout = true;
  
  @Output() itemClick = new EventEmitter<NavigationItemEtablissement>();
  ...
  onItemClick(item: NavigationItemEtablissement) {
  ...
  getItemClasses(item: NavigationItemEtablissement): string {
    const baseClasses = '';
    if (item.id === this.activeItem) {
      return `${baseClasses} bg-green-600 text-white border-r-2 border-green-400 font-semibold shadow-md`;
    }
    return `${baseClasses} text-gray-300 hover:bg-gray-800 hover:text-white`;
  }
}

export interface NavigationItemEtablissement {
  id: string;
  label: string;
